...
```bash
uv tool install git+https://github.com/itscloud0/action-pin-check.git@v0.7.0
...
```bash
  uvx --from git+https://github.com/itscloud0/action-pin-check.git@v0.7.0 \
  action-pin-check .github/workflows --fail-on error
```
...
commit SHAs and installs the released `v0.7.0` package from GitHub.
